...
const PERGUNTAS = [
  { id: 1, texto: "Eu prefiro trabalhar:", opcoes: [
    { valor: 'D', texto: 'Liderando equipes e tomando decisões' },
  ...
  { id: 2, texto: "Em um conflito, eu geralmente:", opcoes: [
    { valor: 'D', texto: 'Enfrento diretamente e resolvo' },
    { valor: 'I', texto: 'Busco harmonizar e conversar' },
    { valor: 'S', texto: 'Evito confronto e busco paz' },
    { valor: 'C', texto: 'Analiso os fatos antes de agir' }
  ]},
  { id: 3, texto: "Meus colegas me descreveriam como:", opcoes: [
  ...
  { id: 4, texto: "Ao tomar decisões importantes:", opcoes: [
    { valor: 'D', texto: 'Decido rapidamente e sigo em frente' },
    { valor: 'I', texto: 'Consulto várias pessoas' },
    { valor: 'S', texto: 'Penso com calma e não me apresso' },
    { valor: 'C', texto: 'Analiso todas as informações disponíveis' }
  ]},
  { id: 5, texto: "O que mais me motiva no trabalho:", opcoes: [
...
const PERFIS = {
  D: { nome: 'Dominante', cor: 'bg-red-100 text-red-700', desc: 'Você é decidido, competitivo e focado em resultados. Gosta de desafios e liderança.' },
  I: { nome: 'Influente', cor: 'bg-yellow-100 text-yellow-700', desc: 'Você é comunicativo, entusiasta e otimista. Motiva pessoas e gosta de trabalhar em equipe.' },
  S: { nome: 'Estável', cor: 'bg-green-100 text-green-700', desc: 'Você é paciente, leal e confiável. Prefere ambientes harmoniosos e estáveis.' },
  C: { nome: 'Conforme', cor: 'bg-blue-100 text-blue-700', desc: 'Você é analítico, preciso e organizado. Valoriza qualidade e segue regras.' }
};

export default function TestePerfilComportamental({ user }) {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const lista = await base44.entities.PerfilComportamental.filter({ user_email: user.email });
        if (lista?.length > 0 && lista[0].resultado) {
          setResultado({
  ...
  const calcularResultado = async (resps) => {
    ...
    try {
      await base44.entities.PerfilComportamental.create({
      ...
      <CardContent className="p-6">
        {salvando ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-600">Analisando seu perfil...</p>
          </div>
        ) : resultado ? (
            ...
            <div className="space-y-2">
              {PERGUNTAS[perguntaAtual].opcoes.map((opcao) => (
                <Button
                  ...
                  variant="outline"
                  className="w-full justify-start text-left h-auto py-4 px-4 rounded-xl hover:bg-indigo-50 hover:border-indigo-300"
                >
