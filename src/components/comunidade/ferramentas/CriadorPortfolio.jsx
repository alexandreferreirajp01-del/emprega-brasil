...
export default function CriadorPortfolio({ user }) {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const lista = await base44.entities.PortfolioItem.filter({ user_email: user.email });
        setProjetos(lista || []);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const adicionarProjeto = async () => {
    ...
    setSalvando(true);
    try {
      const novo = await base44.entities.PortfolioItem.create({
        ...novoProjeto,
              ...
              <Card key={projeto.id} className="rounded-xl overflow-hidden">
                {projeto.imagem_url && (
                  <div className="h-32 bg-slate-100">
                    <img src={projeto.imagem_url} alt={projeto.titulo} className="w-full h-full object-cover" />
                  </div>
                )}
