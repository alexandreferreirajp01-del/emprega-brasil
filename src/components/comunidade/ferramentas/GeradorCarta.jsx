...
A carta deve ser formal, objetiva, com 3-4 parágrafos, demonstrando interesse na vaga e destacando qualidades profissionais.
Não inclua placeholders como [Nome] ou [Data]. Escreva o texto completo.`;
  ...
  const salvarCarta = async () => {
    ...
    setSalvando(true);
    try {
      await base44.entities.CartaApresentacao.create({
    ...
    <Card className="rounded-xl">
      ...
      <CardContent className="p-6 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Textarea value={experiencia} onChange={(e) => setExperiencia(e.target.value)} placeholder="Descreva brevemente sua experiência relevante..." className="rounded-lg min-h-[80px]" />
        </div>
        ...
        <Button onClick={gerarCarta} disabled={gerando || !vaga || !empresa} className="bg-green-600 hover:bg-green-700 rounded-xl">
          ...
          {gerando ? 'Gerando com IA...' : 'Gerar Carta com IA'}
        </Button>

        {carta && (
          <div className="space-y-3 pt-4 border-t">
            <Label>Sua Carta de Apresentação:</Label>
            <Textarea value={carta} onChange={(e) => setCarta(e.target.value)} className="rounded-lg min-h-[250px]" />
