...
export default function SimuladorEntrevista({ user }) {
  ...
  const iniciar = async () => {
    ...
    try {
      const response = await base44.integrations.Core.InvokeLLM({
        prompt: `Gere a primeira pergunta de uma entrevista de emprego para a vaga de "${vaga}". Apenas a pergunta.`,
  ...
  const enviarResposta = async () => {
    ...
    if (perguntaIndex >= 5) {
      try {
        const response = await base44.integrations.Core.InvokeLLM({
          prompt: `Avalie as respostas desta entrevista para "${vaga}":
${novoHistorico.map((h, i) => `P${i+1}: ${h.pergunta}\nR: ${h.resposta}`).join('\n\n')}
Dê nota de 1-10, pontos fortes, a melhorar e dicas.`,
          response_json_schema: {
            type: "object",
            properties: {
              nota: { type: "number" },
      ...
      try {
        const response = await base44.integrations.Core.InvokeLLM({
          prompt: `Entrevista para "${vaga}". Gere a próxima pergunta. Apenas a pergunta.`,
          response_json_schema: {
            type: "object",
            properties: { pergunta: { type: "string" } }
          }
        ...
        {!iniciado ? (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Para qual vaga?</Label>
              <Input value={vaga} onChange={(e) => setVaga(e.target.value)} placeholder="Ex: Atendente" className="rounded-lg" />
            </div>
          ...
          <div className="space-y-4">
            ...
            <Button onClick={enviarResposta} disabled={!resposta.trim() || carregando} className="bg-purple-600 hover:bg-purple-700 rounded-xl">
              ...
              {perguntaIndex >= 5 ? 'Finalizar' : 'Enviar'}
            </Button>
          </div>
        )}
      </CardContent>
