...
const CHECKLIST_ITEMS = [
  ...
  { id: 'pesquisa', label: 'Pesquisei sobre a empresa' },
  ...
  { id: 'portfolio', label: 'Portfólio organizado' },
  { id: 'linkedin', label: 'LinkedIn atualizado' },
  { id: 'referencias', label: 'Referências prontas' },
  { id: 'horario', label: 'Alarme para chegar cedo' },
  { id: 'celular', label: 'Celular carregado/silencioso' },
  { id: 'respostas', label: 'Pratiquei respostas comuns' },
  { id: 'postura', label: 'Revisei postura/linguagem corporal' },
...
export default function ChecklistCandidato({ user }) {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const lista = await base44.entities.ChecklistUsuario.filter({ user_email: user.email });
        if (lista?.length > 0) {
  ...
  const toggleItem = async (itemId) => {
    ...
    try {
      if (checklistId) {
        await base44.entities.ChecklistUsuario.update(checklistId, { items_completos: novosCompletos });
      } else {
        const novo = await base44.entities.ChecklistUsuario.create({
      ...
      <CardContent className="p-6 space-y-4">
        <div className="space-y-2">
          ...
          <div className="h-3 bg-slate-200 rounded-full overflow-hidden">
            <div className="h-full bg-teal-500 transition-all" style={{ width: `${progresso}%` }} />
          </div>
          <p className="text-sm text-slate-500">{completos.length} de {CHECKLIST_ITEMS.length} itens</p>
        </div>
        ...
        <div className="space-y-2">
          {CHECKLIST_ITEMS.map((item) => (
            ...
            >
              <Checkbox checked={completos.includes(item.id)} />
