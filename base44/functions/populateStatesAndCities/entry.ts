...
const statesAndCitiesData = `RO	Alta Floresta D'Oeste
RO	Ariquemes
RO	Cabixi
RO	Cacoal
RO	Cerejeiras
...
PR	Curitibanos
PR	Descanso
PR	Dionísio Cerqueira
PR	Dona Emma
PR	Doutor Pedrinho
PR	Entre Rios
PR	Ermo
PR	Erval Velho
PR	Faxinal dos Guedes
PR	Flor do Sertão
PR	Florianópolis
PR	Formosa do Sul
PR	Forquilhinha
...
PR	Pomerode
PR	Ponte Alta
PR	Ponte Alta do Norte
PR	Ponte Serrada
PR	Porto Belo
PR	Porto União
PR	Pouso Redondo
PR	Praia Grande
PR	Presidente Castello Branco
PR	Presidente Getúlio
PR	Presidente Nereu
PR	Princesa
PR	Quilombo
PR	Rancho Queimado
...
PR	Santa Cecília
PR	Santa Helena
PR	Santa Rosa de Lima
PR	Santa Rosa do Sul
PR	Santa Terezinha
PR	Santa Terezinha do Progresso
PR	Santiago do Sul
PR	Santo Amaro da Imperatriz
PR	São Bernardino
PR	São Bento do Sul
PR	São Bonifácio
PR	São Carlos
PR	São Cristóvão do Sul
PR	São Domingos
PR	São Francisco do Sul
PR	São João do Oeste
PR	São João Batista
PR	São João do Itaperiú
PR	São João do Sul
PR	São Joaquim
PR	São José
PR	São José do Cedro
PR	São José do Cerrito
PR	São Lourenço do Oeste
PR	São Ludgero
PR	São Martinho
PR	São Miguel da Boa Vista
PR	São Miguel do Oeste
PR	São Pedro de Alcântara
PR	Saudades
PR	Schroeder
PR	Seara
PR	Serra Alta
...
Deno.serve(async (req) => {
  try {
    ...
    if (!user || (user.role !== 'admin' && user.subscription_type !== 'admin')) {
      return Response.json({ error: 'Unauthorized - Admin apenas' }, { status: 401 });
    }

    // Limpar cidades existentes
    console.log('🗑️ Limpando cidades antigas...');
    const existingCities = await base44.asServiceRole.entities.City.list('', 10000);
    if (existingCities.length > 0) {
      for (const city of existingCities) {
        await base44.asServiceRole.entities.City.delete(city.id);
      }
      console.log(`✅ ${existingCities.length} cidades antigas removidas`);
    }

    const cities = [];
    const lines = statesAndCitiesData.split('\n');
    ...
    console.log(`📊 Total de cidades para inserir: ${cities.length}`);
    ...
    for (let i = 0; i < cities.length; i += batchSize) {
      ...
      console.log(`✅ Inseridas ${inserted}/${cities.length} cidades...`);
    }
